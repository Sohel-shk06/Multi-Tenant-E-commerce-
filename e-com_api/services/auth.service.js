...
// Register User - Simplified without OTP
export const registerUser = async (userData) => {
  const { name, email, password, role } = userData;
...
// Forgot Password - Simplified (token-based reset link)
export const forgotPasswordService = async (email) => {
  const user = await User.findOne({ email });
  if (!user) {
    throw new ApiError(404, 'User with this email does not exist');
  ...
  console.log(`📧 Password Reset Link: ${config.CLIENT_URL}/reset-password/${resetToken}`);
...
// Simplified email change without OTP
export const requestEmailChangeService = async (userId, newEmail) => {
  ...
  // Directly update email (simplified)
  user.email = newEmail;
  user.isVerified = true;
  await user.save({ validateBeforeSave: false });
